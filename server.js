...
function saveBoard(state) {
  return writeChain = writeChain.then(() => {
    ...
    atomicWrite(BOARD_FILE, payload);
    snapshotBackup();
...
const server = http.createServer(async (req, res) => {
  ...
  if (pathname === '/config.js') {
    ...
    res.end(`window.BOARD_CONFIG = { enabled: true, base: ${JSON.stringify(base)} };`);
    return;
  }
  ...
  if (pathname.startsWith('/api/')) {
    ...
    if (pathname === '/api/health' && req.method === 'GET') {
      ...
      res.end(JSON.stringify({ ok: true, online: sseClients.size }));
      return;
    }
...
server.listen(PORT, '0.0.0.0', () => {
  ...
  console.log(`   永久存储：data/board.json（含 /data/backups 快照）`);
